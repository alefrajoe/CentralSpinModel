...
for file in FILES:
    ...
    with open(DIR + '\\' + file, 'r') as f:
        ...
        # plot the data
        plt.subplot(221)
        plt.plot(data[obs.index(x)]/L, magx, linestyle="-")
        plt.ylabel("<x>", labelpad=-5)
        plt.xlabel("t")
        plt.subplot(222)
        plt.plot(data[obs.index(x)]/L, magy, linestyle="-")
        plt.ylabel("<y>", labelpad=-5)
        plt.xlabel("t")
        plt.subplot(223)
        plt.plot(data[obs.index(x)]/L, magz, linestyle="-")
        plt.ylabel("<z>", labelpad=-5)
        plt.xlabel("t")
        plt.subplot(224)
        plt.plot(data[obs.index(x)]/L, deco, linestyle="-")
        plt.ylabel("D", labelpad=-15)
        plt.xlabel("t")

fig.suptitle("L="+str(int(L[0]))+", g="+str(g[0])+r", $\lambda$="+str(lam[0])+r", $\kappa$="+str(k[0])+", h="+str(h[0])+", p="+str(p[0])+", tm="+str(tm[0]), fontsize=12)
#plt.savefig("MeasRandomWithH.pdf")
plt.show()